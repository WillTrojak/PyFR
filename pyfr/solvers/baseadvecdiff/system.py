...
class BaseAdvectionDiffusionSystem(BaseAdvectionSystem):
    ...
    def _rhs_graphs(self, uinbank, foutbank):
        ...
        for l in k['eles/copy_fpts']:
            g2.add(l, deps=deps(l))
        g2.add_all(k['iint/con_u'], deps=k['eles/copy_fpts'])
        g2.add_all(k['bcint/con_u'], deps=k['eles/copy_fpts'])
        ...
        for l in k['eles/gradcoru_upts_curved']:
            g3.add(l, deps=deps(l, 'eles/tgradcoru_upts'))
        ...
        g4.add_mpi_reqs(m['vect_fpts_send'] + m['vect_fpts_recv'])
        ...
        for l in k['eles/tdivtpcorf']:
            g4.add(l, deps=deps(l, 'eles/tdisf_curved', 'eles/tdisf_linear'))
        g4.commit()

        g5 = self.backend.graph()

        # Compute the common normal flux at our MPI interfaces
        g5.add_all(k['mpiint/artvisc_fpts_unpack'])
        ...
        for l in k['eles/negdivconf']:
            g5.add(l, deps=deps(l, 'eles/tdivtconf'))
        g5.commit()
