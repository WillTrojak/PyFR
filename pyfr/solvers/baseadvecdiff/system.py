...
class BaseAdvectionDiffusionSystem(BaseAdvectionSystem):
    ...
    def _rhs_graphs(self, uinbank, foutbank):
        ...
        for l in k['eles/gradcoru_fpts']:
            if (k['eles/gradcoru_upts_curved'] or
                k['eles/gradcoru_upts_linear']):
                ldeps = deps(l, 'eles/gradcoru_upts_curved',
                             'eles/gradcoru_upts_linear')
            else:
                ldeps = deps(l, 'eles/tdisf_fused_curved',
                             'eles/tdisf_fused_linear')
            g2.add(l, deps=ldeps)

        # Set dependencies for interface exchange
        if k['eles/gradcoru_fpts']:
            ideps = k['eles/gradcoru_fpts']
        elif (k['eles/gradcoru_upts_curved'] or k['eles/gradcoru_upts_linear']):
            ideps = (k['eles/gradcoru_upts_curved'] +
                     k['eles/gradcoru_upts_linear'])
        else:
            ideps = k['eles/tdisf_fused_curved'] + k['eles/tdisf_fused_linear']
        ...
        for l in k['eles/tdivtpcorf']:
            if k['eles/tdisf_curved'] and ['eles/tdisf_linear']:
                g2.add(l, deps=deps(l, 'eles/tdisf_curved',
                                    'eles/tdisf_linear'))
            else:
                g2.add(l, deps=deps(l, 'eles/tdisf_fused_curved',
                                    'eles/tdisf_fused_linear'))
    ...
    def _compute_grads_graph(self, uinbank):
        ...
        # Obtain the physical gradients at the solution points
        for l in k['eles/ggradcoru_upts_curved']:
            g2.add(l, deps=deps(l, 'eles/tgradcoru_upts'))
        for l in k['eles/ggradcoru_upts_linear']:
            g2.add(l, deps=deps(l, 'eles/tgradcoru_upts'))
        g2.commit()
